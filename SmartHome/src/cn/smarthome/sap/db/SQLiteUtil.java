...
public class SQLiteUtil {
	...
	private SQLiteDatabase openDB(String dbName) {
		...
		if (file.exists() == true) {
			return SQLiteDatabase.openDatabase(dbName, null,
	...
	public Cursor openQuery(String dbName, String tableName, String condStr) {
		SQLiteDatabase db = openDB(dbName);
		Cursor cursor = db.query(tableName, null, condStr, null, null, null,
				null);
		cursor.moveToFirst();
		closeDB(db);
		return (cursor);
	...
	public boolean isTableExists(String dbName, String tableName) {
		Cursor cursor = openQuery(dbName, SQLite_MASTER_TABLE, "(tbl_name='"
				+ tableName + "')");
